...
ggsave(
  "fig/herbivory_damage/ext_plot_04.pdf",
  ext_plot_04,
  width = PDF_width,
  height = PDF_height,
  units = "in")
...
ggsave(
  "fig/herbivory_damage/ext_plot_05.pdf",
  ext_plot_05,
  width = PDF_width*1.2,
  height = PDF_height,
  units = "in")
...
glm_herbivory_damage_select <-
  glmmTMB(
    herbivory_percentage_mean ~ Hab+Spec+Treatment+Hab:Treatment+Spec:Treatment,
    data = dataset_herbivory_damage,
    family = "beta_family",
    na.action = "na.fail")
...
glm_herbivory_damage_emmeans <-
  emmeans(
    glm_herbivory_damage_select,
    pairwise~Hab+Spec+Treatment+Hab:Treatment+Spec:Treatment,
    type = "response") 
...
model_plot_01 <-
  ...
  labs(
    x = "Habitat",
    y = "Estimated mean herbivory damage (%)" )+
  scale_fill_manual(values = pallete_1)+
  theme(
    text = element_text(size = text_size),
    legend.position = "right") 
